...
HttpTransact::initialize_state_variables_from_request(State *s, HTTPHdr *obsolete_incoming_request)
{
  ...
  MIMEField *quic_connection_id_header = incoming_request->field_find(
        MIME_FIELD_QUIC_CONNECTION_ID, MIME_LEN_QUIC_CONNECTION_ID);
  MIMEField *quic_stream_id_header = incoming_request->field_find(
        MIME_FIELD_QUIC_STREAM_ID, MIME_LEN_QUIC_STREAM_ID);
  MIMEField *quic_packet_number_header = incoming_request->field_find(
        MIME_FIELD_QUIC_PACKET_NUMBER, MIME_LEN_QUIC_PACKET_NUMBER);
  MIMEField *quic_worker_id_header = incoming_request->field_find(
        MIME_FIELD_QUIC_WORKER_ID, MIME_LEN_QUIC_WORKER_ID);
  MIMEField *five_tuple_header = incoming_request->field_find(
        MIME_FIELD_QUIC_FIVE_TUPLE, MIME_LEN_QUIC_FIVE_TUPLE);
  MIMEField *qsho_secret_header = incoming_request->field_find(
        MIME_FIELD_QSHO_SECRET, MIME_LEN_QSHO_SECRET);

  do {
    if (quic_connection_id_header && quic_stream_id_header &&
            quic_packet_number_header && quic_worker_id_header &&
            five_tuple_header && qsho_secret_header)
    {
      int value_length_return;
      const char *str_qc_id =
        quic_connection_id_header->value_get(&value_length_return);
      s->quic_connection_id_len = value_length_return;
      s->quic_connection_id = (unsigned char *)ats_calloc(1, s->quic_connection_id_len + 1);
      s->quic_connection_id_len = hex_to_bytes(s->quic_connection_id,
        str_qc_id, s->quic_connection_id_len);

      s->quic_stream_id = quic_stream_id_header->value_get_int64();

      int pn_len;
      const char *pn_str;

      pn_str = mime_field_value_get_comma_val(quic_packet_number_header,
        &pn_len, 0);
      if (!pn_str) {
        break;
      }
      s->packet_number = mime_parse_int64(pn_str, pn_str + pn_len);
      ...
      s->packet_number_max = mime_parse_int64(pn_str, pn_str + pn_len);
      //s->lb_send_timestamp = mime_parse_int64(pn_str, pn_str + pn_len);
  
      s->nginx_worker_id = quic_worker_id_header->value_get_uint();

      int ft_len;
      const char *ft_str;
      ft_str = mime_field_value_get_comma_val(five_tuple_header,
        &ft_len, 0);
      if (!ft_str) {
        break;
      }
      s->src_ip = mime_parse_uint(ft_str, ft_str + ft_len);
      ...
      s->dst_ip = mime_parse_uint(ft_str, ft_str + ft_len);
      ...
      s->src_port = (uint16_t)mime_parse_uint(ft_str, ft_str + ft_len);
      ...
      s->dst_port = (uint16_t)mime_parse_uint(ft_str, ft_str + ft_len);

      //parse secret
      int secret_value_len;
      const char *secret_value;
      secret_value = mime_field_value_get_comma_val(qsho_secret_header,
        &secret_value_len, 0);
      if (!secret_value) {
        break;
      }
      s->secret.cipher_id = mime_parse_int(secret_value,
      ...
      s->secret.secret = (unsigned char *)ats_calloc(1, secret_value_len + 1);
      s->secret.secret_len = hex_to_bytes(s->secret.secret,
        secret_value, secret_value_len);
      ...
      s->secret.key = (unsigned char *)ats_calloc(1, secret_value_len + 1);
      s->secret.key_len = hex_to_bytes(s->secret.key, secret_value,
        secret_value_len);
      ...
      s->secret.iv = (unsigned char *)ats_calloc(1, secret_value_len + 1);
      s->secret.iv_len = hex_to_bytes(s->secret.iv, secret_value,
        secret_value_len);
      ...
      s->secret.hp = (unsigned char *)ats_calloc(1, secret_value_len + 1);
      s->secret.hp_len = hex_to_bytes(s->secret.hp, secret_value, secret_value_len);

      s->qsho_transform = true;
    }
  } while (false);

  if (s->qsho_transform) {
      //Note("new request, handoff, stream id %lu", s->quic_stream_id);
  } else {
      //Note("new regular request");
  }
