...
{
  //Note("Cache VC openReadFromWriterFailure %ld %p %s", Thread::get_hrtime(), (void *)&CacheVC::openReadFromWriterFailure, __func__);
  od = nullptr;
  vector.clear(false);
  CACHE_INCREMENT_DYN_STAT(cache_read_failure_stat);
...
{
#if 0
  Note("Cache VC openReadFromWriter %ld, %p, %s",
            Thread::get_hrtime(), (void *)&CacheVC::openReadFromWriter, __func__);
#endif
...
{
#if 0
  Note("Cache VC openReadFromWriterMain %ld %p %s", Thread::get_hrtime(), (void *)&CacheVC::openReadFromWriterMain, __func__);
#endif
  cancel_trigger();
  if (seek_to) {
    vio.ndone = seek_to;
...
CacheVC::openReadReadDone(int event, Event *e)
{
  ...
  //Note("Cache VC openReadReadDone %ld, %p, %s", Thread::get_hrtime(), (void *)&CacheVC::openReadReadDone, __func__);
...
int
CacheVC::openReadMain(int /* event ATS_UNUSED */, Event * print_e)
{
  Event *tmp_trigger = trigger;
  cancel_trigger();
  ...
  IOBufferBlock *b = nullptr;
#if 0
  Note("Cache VC  openReadMain %ld, %ld %ld, %s, trigger %p, event %p, this %p",
            Thread::get_hrtime(), ntodo, bytes, __func__, tmp_trigger, print_e, this);
#endif
...
{
#if 0
  Note("Cache VC openReadStartEarliest %ld %p %s", Thread::get_hrtime(), (void *)&CacheVC::openReadStartEarliest, __func__);
#endif
  int ret  = 0;
  Doc *doc = nullptr;
  cancel_trigger();
...
{
#if 0
  Note("Cache VC openReadVecWrite %ld, %p %s", Thread::get_hrtime(), (void *)&CacheVC::openReadVecWrite, __func__);
#endif
  cancel_trigger();
  set_io_not_in_progress();
  ink_assert(od);
...
CacheVC::openReadStartHead(int event, Event *e)
{
  ...
  cancel_trigger();
#if 0
  Note("Cache VC openReadStartHead %ld %p %s", Thread::get_hrtime(), (void *)&CacheVC::openReadStartHead, __func__);
#endif
  set_io_not_in_progress();
  if (_action.cancelled) {
    return free_CacheVC(this);
...
{
#if 0
  Note("Cache VC openReadDirDelete %ld, %p, %s", Thread::get_hrtime(), (void *)&CacheVC::openReadDirDelete, __func__);
#endif
