...
HttpSM::HttpSM() : Continuation(nullptr), vc_table(this) {
  ...
  init_timestamp = ts.tv_sec * 1000000000 + ts.tv_nsec;
}
...
int
skip_ack_msg(IOBufferReader *buf_reader)
{
    uint32_t total_len;
    uint32_t type;
    char method_buf[8];
    ...
    do {
        if (!buf_reader->is_read_avail_more_than(3)) {
            if (buf_reader->read_avail() >= 1) {
                buf_reader->memcpy(method_buf, buf_reader->read_avail());
                if (!(method_buf[0] ^ 'G')) {
                    return 0;
                }
            }
        ...
        buf_reader->memcpy(method_buf, 3);
        if (!(method_buf[0] ^ 'G') && !(method_buf[1] ^ 'E') && !(method_buf[2] ^ 'T')) {
            //GET请求，返回0正常解析请求
            return 0;
        }
        //Ack Msg, 跳过
        if (!buf_reader->is_read_avail_more_than(8)) {
            Note("non HTTP request, wait 8 byte");
            return 1;
        }
        buf_reader->memcpy(method_buf, 8);
        memcpy(&total_len, method_buf, 4);
        memcpy(&type, method_buf + 4, 4);
        ink_assert(type == ACK_MSG && total_len == sizeof(qsho_ack_message_t));
        if (!buf_reader->is_read_avail_more_than(total_len - 1)) {
            Note("skip not satisfy acm msg %u", total_len);
            return 1;
        }
        buf_reader->consume(total_len);
    } while (true);
}
...
HttpSM::state_read_client_request_header(int event, void *data)
{
  ...
  // tokenize header //
  /////////////////////
  ParseResult state = PARSE_RESULT_DONE;
  if (((event == VC_EVENT_READ_READY) || (event == VC_EVENT_READ_READY)) && !have_skip_ack) {
      if (skip_ack_msg(ua_buffer_reader) == 1) {
...
int
HttpSM::tunnel_handler_qsho_ua(int event, HttpTunnelConsumer *c)
{
  ...
  QshoStream *qs = dynamic_cast<QshoStream *>(qsho_info.vc);
  //qs->do_io_close();
  qs->do_io_shutdown(IO_SHUTDOWN_WRITE);
  do_qsho_transform_close();
  ...
  if (close_connection) {
    ...
    //Note("qsho connection keepalive ua_txn %p, proxy ssn %p", ua_txn, ua_txn->get_proxy_ssn());
...
VConnection *
HttpSM::do_qsho_transform_open()
{
    if (!t_state.qsho_transform) {
        return nullptr;
    }
    QshoStream *qs = new QshoStream(t_state.quic_connection_id,
            t_state.quic_connection_id_len, t_state.quic_stream_id,
            t_state.nginx_worker_id, t_state.packet_number, t_state.packet_number_max,
            t_state.lb_send_timestamp, t_state.src_ip, t_state.dst_ip,
            t_state.src_port, t_state.dst_port, &t_state.secret);
    t_state.quic_connection_id = nullptr;
    t_state.quic_connection_id_len = 0;
    qsho_info.vc = qs;
    qsho_info.entry = vc_table.new_entry();
    qsho_info.entry->vc = qsho_info.vc;
    qsho_info.entry->vc_type = HTTP_UA_VC;
    return qs;
}

void
HttpSM::do_qsho_transform_close()
{
    qsho_info.vc = nullptr;
    vc_table.remove_entry(qsho_info.entry);
    qsho_info.entry = nullptr;
}
...
int
HttpSM::write_qsho_http3_header_into_buffer(HTTPHdr *h, MIOBuffer *b)
{
    unsigned char *tmp_buf = (unsigned char *)ats_calloc(1, 4096);
    unsigned char *p = tmp_buf + 16;

    p = (u_char *) ngx_http_v3_encode_field_section_prefix(p, 0, 0, 0);
    HTTPStatus hs = h->status_get();
    if (hs == HTTP_STATUS_OK) {
        p = (u_char *) ngx_http_v3_encode_field_ri(p, 0, 25);
    } else {
        p = (u_char *) ngx_http_v3_encode_field_lri(p, 0, 25, NULL, 3);
        p += sprintf(((char *)p), "%03u", ((uint32_t)hs)); 
    }

    MIMEFieldBlockImpl *fblock;
    MIMEField *field;
    uint32_t index;
    MIMEHdrImpl *mh = h->m_http->m_fields_impl;
    unsigned char *server_data = (unsigned char *)"nginx-qsho";
    uint32_t server_data_len = sizeof("nginx-qsho") - 1;
    p = (u_char *)ngx_http_v3_encode_field_lri(p, 0, 92, server_data,
            server_data_len);
    for (fblock = &(mh->m_first_fblock); fblock != nullptr;
            fblock = fblock->m_next)
    {
        for (index = 0; index < fblock->m_freetop; index++) {
            ...
            if (field->m_wks_idx == MIME_WKSIDX_SERVER) {
                continue;
            }

            if (field->m_wks_idx == MIME_WKSIDX_AGE) {
                continue;
            }

            if (field->m_wks_idx == MIME_WKSIDX_DATE) {
                p = (u_char *)ngx_http_v3_encode_field_lri(p, 0, 6, (u_char *)field->m_ptr_value,
                        field->m_len_value);
                continue;
            }

            if (field->m_wks_idx == MIME_WKSIDX_CONTENT_TYPE) {
                p = (u_char *)ngx_http_v3_encode_field_lri(p, 0, 53, (u_char *)field->m_ptr_value,
                        field->m_len_value);
                continue;
            }

            if (field->m_wks_idx == MIME_WKSIDX_CONNECTION) {
                continue;
            }

            p = (u_char *) ngx_http_v3_encode_field_l(p, (u_char *)field->m_ptr_name,
                    field->m_len_name, (u_char *)field->m_ptr_value, field->m_len_value);
            //Note("header: %s, value: %s", field->m_ptr_name, field->m_ptr_value);
        }
    }
    uint32_t n = p - (tmp_buf + 16);
    uint32_t frame_n = ngx_http_v3_encode_varlen_int(nullptr, 0x01) +
            ngx_http_v3_encode_varlen_int(nullptr, n);
    ink_assert(frame_n <= 16);
    unsigned char *frame_p = tmp_buf + (16 - frame_n);
    unsigned char *data_start = frame_p;
    //quic frame header
    frame_p = (u_char *)ngx_http_v3_encode_varlen_int(frame_p, 0x01);
    frame_p = (u_char *)ngx_http_v3_encode_varlen_int(frame_p, n);
    ink_assert(frame_p == (tmp_buf + 16));

    //quic frame data
    int64_t doc_size;
    doc_size = t_state.cache_info.object_read->object_size_get();
    p = (u_char *)ngx_http_v3_encode_varlen_int(p, 0x00);
    p = (u_char *)ngx_http_v3_encode_varlen_int(p, doc_size);

    int64_t total_header_len = p - data_start;
    int64_t header_offset = 0;
    while (header_offset < total_header_len) {
        IOBufferBlock *block = b->get_current_block();
        ink_assert(block->write_avail() > 0);
        uint32_t now_copy = block->write_avail();
        if (now_copy > (total_header_len - header_offset)) {
            now_copy = total_header_len - header_offset;
        }
        memcpy(block->start(), data_start + header_offset, now_copy);
        header_offset += now_copy;
        b->fill(now_copy);

        if (block->write_avail() == 0 && header_offset < total_header_len) {
            b->add_block();
        }
    }
    ats_free(tmp_buf);
...
HttpSM::attach_server_session(PoolableSession *s)
{
  ...
  server_protocol = server_session->get_protocol_string();
...
HttpSM::setup_cache_read_transfer()
{
  ...
  t_state.hdr_info.client_response.set_timestamp1(init_timestamp);
  t_state.hdr_info.client_response.set_timestamp2(timestamp);
  ...
  if (!t_state.qsho_transform || !do_qsho_transform_open()) {
    tunnel.add_consumer(ua_entry->vc, cache_sm.cache_read_vc, &HttpSM::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
  } else {
    HttpTunnelConsumer *c;
    tunnel.qsho_transform = true;
    c = tunnel.add_consumer(qsho_info.vc, cache_sm.cache_read_vc, &HttpSM::tunnel_handler_qsho_ua, HT_HTTP_CLIENT, "user qsho agent");
    qsho_info.entry->in_tunnel = true;
    p->qsho_transform = true;
    p->qshoctrl = new QshoCtrl(p, c, dynamic_cast<QshoStream *>(qsho_info.vc),
        dynamic_cast<VConnection *>(ua_entry->vc));
    QshoStream *qs = (QshoStream *)qsho_info.vc;
    qs->total_len = doc_size;
    qs->header_size = hdr_size;
    qs->qshoctrl = p->qshoctrl;
    //p->qshoctrl->send_b2f_msg(0);
  }
