...
class QshoCtrl : public Continuation {
public:
    QshoCtrl(HttpTunnelProducer *_p, HttpTunnelConsumer *_c,
    ...
    ink_hrtime           rs_last_recv_timestamp;
    bool                 eos = false;
    bool                 client_finished = false;

    QshoCtrl();
    void send_request_info(uint64_t total_len, uint32_t pn_len);
    int main_handler(int event, void *data);
    void qsho_parse_read_message();
    void send_b2f_msg(uint64_t pn);

    MIOBuffer *read_buffer;
    MIOBuffer *write_buffer;

    IOBufferReader *buf_reader;
    IOBufferReader *write_buf_reader;
};
...
struct HttpTunnelProducer {
  ...
  bool qsho_transform = false;
  QshoCtrl *qshoctrl = nullptr;
...
public:
  ...
  int qsho_ctrl_handler(int event, QshoCtrl *qshoctrl);
...
inline QshoCtrl *
HttpTunnel::get_qshoctrl(VIO *vio)
{
  for (int i = 0; i < MAX_PRODUCERS; i++) {
    if (producers[i].qshoctrl && 
        (producers[i].qshoctrl->read_vio == vio ||
        producers[i].qshoctrl->write_vio == vio))
    {
      return producers[i].qshoctrl;
    }
  }
  return nullptr;
}
