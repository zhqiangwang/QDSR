...
QshoCtrl::QshoCtrl(HttpTunnelProducer *_p, HttpTunnelConsumer *_c,
        QshoStream *_qs, VConnection *_vc)
{
    SET_HANDLER(&QshoCtrl::main_handler);
    int64_t alloc_index = MAX_IOBUFFER_SIZE;

    //read_buffer = new_MIOBuffer(alloc_index);
    write_buffer = new_MIOBuffer(alloc_index);

    //buf_reader = read_buffer->alloc_reader();
    write_buf_reader = write_buffer->alloc_reader();
    p = _p;
    c = _c;
    qs = _qs;
    vc = _vc;
    ...
    ProxySession *proxy_ssn = (dynamic_cast<ProxyTransaction *>(vc))->get_proxy_ssn();
    Http1ClientSession *css = dynamic_cast<Http1ClientSession *>(proxy_ssn);
    //Note("qshoctrl %p proxy ssn %p %p", proxy_ssn, css->read_buffer, css->_reader);
    buf_reader = css->_reader;
    read_buffer = css->read_buffer;
    //read_buffer = proxy_ssn->read_buffer;

    read_vio = vc->do_io_read(this, INT64_MAX, css->read_buffer);
}

QshoCtrl::~QshoCtrl()
{
    ink_assert(qs->ack_finished);
    read_vio = vc->do_io_read(this, 0, nullptr);
    write_vio = vc->do_io_write(this, 0, nullptr);
    mutex = nullptr;
...
int
QshoCtrl::main_handler(int e, void *data)
{
    Event *event = (Event *)data;
    QshoCtrl *qshoctrl;
    VIO *now_vio = (VIO*)data;
    ACKFrame *qac;
    switch (e) {
    case VC_EVENT_READ_READY:
    case VC_EVENT_READ_COMPLETE:
        qsho_parse_read_message();
        break;
    ...
    case VC_EVENT_WRITE_COMPLETE:
        //Note("qsho ctrl write complete");
        //if (qs->ack_finished && client_finished) {
        do {
    ...
    case VC_EVENT_EOS:
        Note("qsho ctrl eos, %lu", qs->stream_id);
#if 0
#endif
        eos = true;
...
void
QshoCtrl::qsho_parse_read_message()
{
    int i;
    uint32_t total_len;
    uint32_t type;
    qsho_ack_message_t qam;
    ACKFrame *qac = nullptr;
    int ack_num;

    qsho_f2b_t b2f;
    qsho_f2b_t f2b;

    if (read_buffer->current_write_avail() < 1024) {
        read_buffer->add_block();
    }
    ...
    while (true) {
        ...
        rs_last_recv_timestamp = Thread::get_hrtime();
        buf_reader->memcpy(padding, 8);
        memcpy(&total_len, padding, 4);
        memcpy(&type, padding + 4, 4);
        ink_assert(total_len > 8);
        if (!buf_reader->is_read_avail_more_than(total_len - 1)) {
            return;
        }
        switch (type) {
        case ACK_MSG:
            ack_num = ((total_len - sizeof(qsho_ack_message_t)) / 16);
            buf_reader->read(&qam, sizeof(qsho_ack_message_t));
            qac = new ACKFrame();
            qac->ack_min = qam.ack_min;
            qac->ack_max = qam.ack_max;
            qac->pn_min = qam.pn_min;
            qac->pn_max = qam.pn_max;
            qac->ack_delay = qam.client_ack_delay;
            qac->lb_ack_delay = qam.lb_send_timestamp - qam.lb_recv_timestamp;
            qac->recv_timestamp = rs_last_recv_timestamp;
            for (i = 0; i < ack_num; i ++) {
                ACKRange *range = new ACKRange();
                uint64_t ack_pn;
                buf_reader->read(&ack_pn, sizeof(uint64_t));
                range->ack_min = ack_pn;
                buf_reader->read(&ack_pn, sizeof(uint64_t));
                range->ack_max = ack_pn;
                qac->ack_ranges.enqueue(range);
                range = nullptr;
            }
            //Note("get ack qsid %lu, [%lu - %lu]", qs->stream_id, qac->ack_max, qac->ack_min);

            lb_send_timestamp = qam.lb_send_timestamp;
            qs->atomic_acks.push(qac);
            qac = nullptr;
            break;
        ...
        //TODO
        case F2B_MSG:
            buf_reader->read(&f2b, sizeof(qsho_ack_message_t));
            //Note("get f2b finished mesg");
            if (f2b.finished) {
                ink_assert(qs->ack_finished);
                client_finished = true;
                MUTEX_TRY_LOCK(lock, qs->write.vio.mutex, qs->thread);
        ...
        qs->thread->schedule_imm_local(qs, VC_EVENT_WRITE_READY, qs);
    }
}

void
QshoCtrl::send_b2f_msg(uint64_t pn)
{
    qsho_b2f_t b2f;
    memset(&b2f, 0, sizeof(b2f));
    b2f.total_len = sizeof(b2f);
    b2f.type = B2F_MSG;
    b2f.content_total_len = qs->total_len;
    b2f.finished = qs->ack_finished;
    b2f.rs_send_timestamp = Thread::get_hrtime();
    b2f.rs_last_recv_timestamp = rs_last_recv_timestamp;
    b2f.lb_send_timestamp = lb_send_timestamp;
    b2f.unused_pn = pn;

    MUTEX_TRY_LOCK(lock, mutex, qs->thread);
    int64_t sz = write_buffer->write((void *)&b2f, sizeof(b2f));
    write_vio = vc->do_io_write(this, sz, write_buf_reader);
}
  ...
  if (p && p->alive) {
    ...
    if (backlog >= flow_state.high_water) {
      ...
      if (srcp && srcp->alive && c->is_sink()) {
        ...
        Note("backlog2 %lu, low water %lu high water %lu srcp %p", backlog,
                flow_state.low_water, flow_state.high_water, srcp);
        if (backlog < flow_state.low_water) {
          ...
          if (HT_HTTP_CLIENT == c->vc_type && !qsho_transform) {
          ...
          } else if (HT_HTTP_CLIENT == c->vc_type && qsho_transform) {
            QshoStream *qs = dynamic_cast<QshoStream *>(c->write_vio->vc_server);
            qs->trapWriteBufferEmpty();
          }
        }
      }
      uint64_t before_read_avail, after_read_avail;
      QshoStream *qs = dynamic_cast<QshoStream *>(c->vc);
      if (qs) {
        before_read_avail = qs->write.vio.buffer.reader()->read_avail();
      }
      if (p->read_vio) {
        p->read_vio->reenable();
      }
      ...
      if (qs) {
        after_read_avail = qs->write.vio.buffer.reader()->read_avail();
        //Note("cachevc reenable before %lu, after %lu", before_read_avail, after_read_avail);
      }
    }
  }
}
...
int
HttpTunnel::qsho_ctrl_handler(int event, QshoCtrl *qshoctrl)
{
#if 0
  HttpTunnelProducer *p = qshoctrl->p;
  HttpTunnelConsumer *c = qshoctrl->c;
#endif
  switch (event) {
  case VC_EVENT_WRITE_READY:
    break;
...
HttpTunnel::consumer_handler(int event, HttpTunnelConsumer *c)
{
  ...
  HttpTunnelProducer *p = c->producer;
  QshoCtrl *qsho_ctrl = nullptr;
  if (qsho_transform) {
      qsho_ctrl = p->qshoctrl;
  }
  ...
  // Find the appropriate entry
    if ((p = get_producer(static_cast<VIO *>(data))) != nullptr) {
        ...
        if ((c = get_consumer(static_cast<VIO *>(data))) != nullptr) {
            ...
            /*** why going here ***/
            //Note("In Tunnel main handler, ctrl branch, not possible");
            if (qsho_transform && data) {
                for (int i = 0; i < MAX_CONSUMERS; i++) {
                    void *tmp_p = (void *)(&(((QshoStream*)(consumers[i].vc))->write.vio));
                    if (consumers[i].alive && tmp_p == data) {
                        c = consumers + i;
                        found = true;
                    }
                }
            ...
            if (!found ) {
                //qsho control
                QshoCtrl *qshoctrl = get_qshoctrl(static_cast<VIO *>(data));
                ink_assert(qshoctrl);
                qsho_ctrl_handler(event, qshoctrl);
            }
        }
    }
