...
static void
handle_ack_mp_frame_range(ngx_quic_stream_t *qs, uint64_t min, uint64_t max);
    ...
    ngx_quic_frame_t *f)
{
    ...
    ngx_log_error(NGX_LOG_ERR, c->log, 0,
            "largest %uL, delay %uL, range_count %uL, first_range %uL, ranges_length %uL",
            ack->largest, ack->delay, ack->range_count, ack->first_range,
            ack->ranges_length);
#endif
    ...
    if (ack->pnsid != UINT64_MAX) {
        ngx_queue_t *q;
        if (ngx_queue_empty(&qc->qsho_streams)) {
            return NGX_OK;
        }    
        q = ngx_queue_head(&qc->qsho_streams);

        while (q && q != ngx_queue_sentinel(&qc->qsho_streams)) {
            qs = ngx_queue_data(q, ngx_quic_stream_t, qsho_qc_queue);
            q = ngx_queue_next(q);
            if (qs->dsr_cid->seqnum == ack->pnsid) {
                break;
            }
            qs = NULL;
        }
        if (qs == NULL) {
            ngx_log_error(NGX_LOG_ERR, c->log, 0, "ACK_MP can not find qs, pnsid %uL", ack->pnsid);
            return NGX_OK;
        }
    }

    if (ack->pnsid != UINT64_MAX) { 
        handle_ack_mp_frame_range(qs, min, max);
    } else  {
...
#if 0
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "range count %uL, pnsid %uL max:min %uL %uL",
            ack->range_count, ack->pnsid, max, min);
#endif
    ...
    for (i = 0; i < ack->range_count; i++) {
        ...
        min = max - range;
    //    ngx_log_error(NGX_LOG_ERR, c->log, 0,
    //                  "i %ui min %uL max %uL", i, min, max);

        if (ack->pnsid != UINT64_MAX) { 
            handle_ack_mp_frame_range(qs, min, max);
        } else if (ngx_quic_handle_ack_frame_range(c, ctx, min, max, &send_time)
            != NGX_OK)
        {
            return NGX_ERROR;
        }
    ...
    if (ack->pnsid != UINT64_MAX) {
        ngx_msec_t ack_delay = ack->delay * (1 << qc->ctp.ack_delay_exponent) / 1000;
        qsho_send_ack(c, qs, ack_delay * 1000000);
        return NGX_OK;
    }
...
handle_ack_mp_frame_range(ngx_quic_stream_t *qs, uint64_t min, uint64_t max)
{
    ...
    qae = ngx_array_push(qs->ack_array);
    qae->ack_min = min;
    qae->ack_max = max;
}
    ...
    uint64_t min, uint64_t max, ngx_quic_ack_stat_t *st)
{
...
#if 0
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "%uL %uL\n",
    ...
    if (!found) {
        ...
        ngx_log_error(NGX_LOG_INFO, c->log, 0,
                      "quic ACK for the packet not sent %uL %uL", max, ctx->pnum);
...
ngx_quic_is_blocked(ngx_connection_t *c)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                   "block return 1 win:%uz ss:%z if:%uz",
...
ngx_quic_congestion_ack(ngx_connection_t *c, ngx_quic_frame_t *f)
{
    ...
    if (cg->window < cg->ssthresh) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                       "quic congestion slow start window:%uz ssthresh:%z in_flight:%uz",
                       cg->window, cg->ssthresh, cg->in_flight);
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0,
                       "quic congestion avoidance window:%uz ssthresh:%z in_flight:%uz",
                       cg->window, cg->ssthresh, cg->in_flight);
    }
