...
static void qsho_upstream_read_handler(ngx_http_request_t *r,
        ngx_http_upstream_t *u);
...
void
qsho_upstream_finalize_request(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ngx_log_debug1(NGX_LOG_ERR, r->connection->log, 0,
                   "qsho finalize http upstream request %p\n", r);
    ...
    *u->cleanup = NULL;
...
#if 0
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
            "qsho finalize upstream qsid %uL output %s",
            r->connection->quic->id, u->qsho_out_bufs ? "true" : "false");
#endif
    ...
    u->finalize_request(r, NGX_ERROR);
...
void
dsr_init(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ngx_connection_t  *c;
    c = u->peer.connection;
    u->dsr_inited = 1;
    ...
    u->output.output_filter = ngx_chain_writer;
    u->output.filter_ctx = &u->writer;
    u->write_event_handler = qsho_upstream_write_handler;
    u->read_event_handler = qsho_upstream_read_handler;

    if (ngx_handle_read_event(c->read, 0) != NGX_OK) {
        qsho_upstream_finalize_request(r, u);
        return;
    }
...
void
qsho_upstream_write_handler(ngx_http_request_t *r,
        ngx_http_upstream_t *u)
{
    ngx_chain_t               *out;
    ngx_chain_t               *cl;
    ...
    wev = c->write;
#if 0
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "qsho write handler r %p, u %p\n",
        r, u);
#endif
    ...
    if (wev->timedout) {
        ...
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "qsho write handler not ready and "
                "timeout %s\n", wev->active ? "active" : "not active");
    }

    out = u->qsho_out_bufs;
    u->qsho_out_bufs = NULL;
#if 0
    if (out == NULL) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "qsho write handler not data write\n");
        goto ok_exit;
    }

    static int count = 0;
    count ++;
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "write chain count %d\n", count);
#endif
    ...
    if (rc == NGX_ERROR) {
        static int count = 0;
        count ++;
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "qsho write error count %d\n", count);
        qsho_upstream_finalize_request(r, u);
        return;
    }
    ...
    if (rc == NGX_AGAIN) {
        while (out) {
            if (((ngx_chain_writer_ctx_t *)u->output.filter_ctx)->out &&
                    out->buf == ((ngx_chain_writer_ctx_t *)u->output.filter_ctx)->out->buf)
            {
                break;
            }
            cl = out;
            //cl->buf to free
            out = out->next;
            free_qsho_buf_chain(cl);
            cl = NULL;
        }

        static int count = 0;
        count ++;
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "qsho write again count %d\n", count);
...
static void
qsho_upstream_read_handler(ngx_http_request_t *r,
        ngx_http_upstream_t *u)
{
    ssize_t            n;
    ngx_int_t          rc;
    ...
    c = u->peer.connection;
#if 0
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "read handler r %p, u %p\n",
        r, u);
#endif
    ...
    if (c->read->timedout) {
        qsho_upstream_finalize_request(r, u);
        return;
    } else if (!c->read->ready) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "qsho read handler not ready and "
                "timeout %s\n", c->read->active ? "active" : "not active");
    }
    ...
    for ( ;; ) {
        ...
        u->buffer.last += n;
        rc = qsho_parse_request(r, &u->buffer);
...
ngx_int_t
qsho_parse_request(ngx_http_request_t *r, ngx_buf_t *b)
{
    ...
    switch (cmd_type) {
    case B2F_MSG:
        msg = (qsho_b2f_t *)b->pos;
        if (msg->finished) {
            qs->dsr_cid->unused_pn = msg->unused_pn;
    ...
    if (rc == NGX_DONE && WRITE_RESPONSE) {
        ...
        *ll = ack_chain;
        ...
        qsho_f2b_t *f2b;
        f2b = (qsho_f2b_t *) ack_buf->last;
        f2b->total_len = sizeof(*f2b);
        f2b->type = F2B_MSG;
        ack_buf->last += sizeof(*f2b);
        f2b->finished = 1;
...
ngx_http_upstream_cleanup(void *data)
{
    ...
    if (r->upstream->upstream_handoff) {
        qsho_upstream_finalize_request(r, r->upstream);
    } else {
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
            "finalize upstream qsid %uL output %s",
            r->connection->quic->id, u->qsho_out_bufs ? "true" : "false");
