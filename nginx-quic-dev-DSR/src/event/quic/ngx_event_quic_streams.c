...
ngx_quic_create_stream(ngx_connection_t *c, uint64_t id)
{
    ...
    *log = *c->log;
    ...
    qs->ack_array = ngx_array_create(pool, 16, sizeof(qsho_ack_entry_t));
...
ngx_quic_stream_cleanup_handler(void *data)
{
    ...
    if (qs->dsr_cid) {
        ...
        pt = ngx_hex_dump(tmp_buf, qs->dsr_cid->id, qs->dsr_cid->len);
        snprintf((char *)pt, tmp_buf + sizeof(tmp_buf) - pt, " dsr finished qsid %lu, pnsid %lu",
                qs->id, qs->dsr_cid->seqnum);
