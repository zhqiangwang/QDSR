...
static ngx_command_t  ngx_http_proxy_commands[] = {
    ...
    { ngx_string("handoff"),
      ...
      ngx_conf_set_flag_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_proxy_loc_conf_t, handoff),
      NULL },
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    if (plcf->handoff && r->connection->quic) {
        u->handoff = 1;
    } else {
        u->handoff = 0;
    }
    u->upstream_handoff = 0;
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    u_char                        handoff_header_buf[1024];
    u_char                       *p, *old_p;
    size_t                        handoff_header_len;
    ...
    if (u->handoff) {
        cid = allocate_dsr_client_cid(r->connection);
        if (cid == NULL) {
            ngx_connection_t       *c = r->connection;
            ngx_quic_stream_t *qs = c->quic;
            u->handoff = 0;
            ngx_log_error(NGX_LOG_ERR, c->log, 0,
                   "cannot alloc dsr cid %uL", qs->id);
        }
    }
    ...
    if (u->handoff) {
        static u_char                  tmp_buf[1024];
        static u_char                  tmp_buf2[1024];
        static u_char                  tmp_buf3[1024];
        static u_char                  tmp_buf4[1024];
        ...
        ngx_queue_insert_tail(&qc->qsho_streams, &qs->qsho_qc_queue);
        qs->data = r;
        u->upstream_handoff = 1;
        //u->keepalive = 1;
        qs->handoff = 1;

        handoff_header_len = 0;
        old_p = handoff_header_buf;

        ngx_memzero(&tmp_buf, sizeof(tmp_buf));
        ngx_hex_dump(tmp_buf, cid->id, cid->len);
        p = ngx_snprintf(old_p, sizeof(handoff_header_buf) - handoff_header_len,
                "QUIC-Connection-Id: %s" CRLF, tmp_buf);
        handoff_header_len += p - handoff_header_buf;
        old_p = p;

        p = ngx_snprintf(old_p, sizeof(handoff_header_buf) - handoff_header_len,
                "QUIC-Stream-Id: %uL" CRLF, c->quic->id);
        handoff_header_len += p - old_p;
        old_p = p;

        p = ngx_snprintf(old_p, sizeof(handoff_header_buf) - handoff_header_len,
                "QUIC-Packet-Number: %uL, %uL" CRLF, cid->unused_pn, 0);
        handoff_header_len += p - old_p;
        old_p = p;

        p = ngx_snprintf(old_p, sizeof(handoff_header_buf) - handoff_header_len,
                "QUIC-Worker-Id: %lu" CRLF, cid->seqnum);
        handoff_header_len += p - old_p;
        old_p = p;
        ...
        uint16_t src_port;
        uint16_t dst_port;
        uint32_t output_src_port;
        uint32_t output_dst_port;
        struct sockaddr_in *addr = (struct sockaddr_in *)pc->local_sockaddr;
        src_ip = addr->sin_addr.s_addr;
        src_port = addr->sin_port;

        addr = (struct sockaddr_in *)qc->socket->path->sockaddr;
        dst_ip = addr->sin_addr.s_addr;
        dst_port = addr->sin_port;

        output_src_port = src_port;
        output_dst_port = dst_port;
        p = ngx_snprintf(old_p, sizeof(handoff_header_buf) - handoff_header_len,
                "QUIC-Five_Tuple: %uD,%uD,%uD,%uD" CRLF,
                src_ip, dst_ip, output_src_port, output_dst_port);
        handoff_header_len += p - old_p;
        old_p = p;

        ngx_quic_secret_t *secret;
        secret = &qc->keys->secrets[ssl_encryption_application].server;
        ngx_memzero(&tmp_buf, sizeof(tmp_buf));
        ngx_memzero(&tmp_buf2, sizeof(tmp_buf2));
        ngx_memzero(&tmp_buf3, sizeof(tmp_buf3));
        ngx_memzero(&tmp_buf4, sizeof(tmp_buf4));
        ngx_hex_dump(tmp_buf, secret->secret.data, secret->secret.len);
        ngx_hex_dump(tmp_buf2, secret->key.data, secret->key.len);
        ngx_hex_dump(tmp_buf3, secret->iv.data, secret->iv.len);
        ngx_hex_dump(tmp_buf4, secret->hp.data, secret->hp.len);
        p = ngx_snprintf(old_p, sizeof(handoff_header_buf) - handoff_header_len,
                "Qsho-Secret: %d,%s,%s,%s,%s" CRLF,
                qc->keys->cipher, tmp_buf, tmp_buf2, tmp_buf3, tmp_buf4);
        handoff_header_len += p - old_p;
        old_p = p;

        len += handoff_header_len;
    }
    ...
    *b->last++ = ' ';
    ...
    if (u->handoff) {
        b->last = ngx_copy(b->last, handoff_header_buf, handoff_header_len);
    }
    ...
    if (rc == NGX_ERROR && u->handoff) {
        ...
        if (rc == QSHO_OK) {
            ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0,
                      "upstream handoff success %uL", qc->avg_rtt);
            return rc;
        } else {
            if (qs->qsho_qc_queue.next) {
                ngx_queue_remove(&qs->qsho_qc_queue);
            }
            return NGX_ERROR;
        }
    }
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->handoff, prev->handoff, 0);
