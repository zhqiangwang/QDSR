...
#define lost_threshold(qs)                                           \
    ngx_max(4 * ngx_max((qs)->latest_rtt, (qs)->avg_rtt), 1)
    ...
    ngx_quic_header_t *pkt)
{
    ...
    qc->tp.active_connection_id_limit = 16;
    ctp = &qc->ctp;
    ...
    ctp->active_connection_id_limit = 16;
    ...
    ngx_queue_init(&qc->qsho_streams);

    ngx_log_error(NGX_LOG_ERR, c->log, 0, "new connection %d\n", ngx_worker);
    return qc;
}
...
void
qsho_send_ack(ngx_connection_t *c, ngx_quic_stream_t *qs, uint64_t client_ack_delay)
{
    ngx_buf_t *ack_buf;
    ngx_chain_t **ll;
    ngx_chain_t *cl, *ack_chain;
    ngx_http_request_t *r;
    ngx_http_upstream_t *u;
    static uint64_t tmp_buf[512];
    int32_t ack_range_count = 0;
    uint64_t total_len;
    qsho_ack_message_t *qam = NULL;
    qsho_ack_entry_t *qae;
    ngx_uint_t i;
#ifdef ACK_DEBUGBUF
    u_char *pt;
    static unsigned char debugbuf[4096];
    pt = debugbuf;
    memset(debugbuf, 0, sizeof(debugbuf));
#endif

    r = (ngx_http_request_t *)qs->data;
    if (r == NULL) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "in send ack, the r is NULL %uL", qs->id);
        return;
    }
    u = r->upstream;
    ...
    qae = qs->ack_array->elts;

    if (qs->ack_array->nelts == 0) {
        return;
    }

    for (i = 0; i < qs->ack_array->nelts; i ++) {
        tmp_buf[2 * ack_range_count] = qae[i].ack_min;
        tmp_buf[2 * ack_range_count + 1] = qae[i].ack_max;
        ack_range_count ++;
#ifdef ACK_DEBUGBUF
        pt = ngx_snprintf(pt, debugbuf + sizeof(debugbuf) - pt, "[%uL - %uL], ",
                qae[i].ack_max, qae[i].ack_min);
#endif
    }

    qs->ack_array->nelts = 0;

    total_len = sizeof(qsho_ack_message_t) + (ack_range_count - 1) * 2 * sizeof(uint64_t);
    assert(total_len <= 4096);

    if (u->qsho_out_bufs) {
        for (cl = u->qsho_out_bufs; cl->next; cl = cl->next) { /* void */ }
        ll = &cl->next;
    } else {
        ll = &u->qsho_out_bufs;
    }
    ack_chain = alloc_qsho_buf_chain_buf();
    *ll = ack_chain;
    ack_buf = ack_chain->buf;

    qam = (qsho_ack_message_t *)ack_buf->last;
    ack_buf->last += total_len;


    qam->total_len = total_len;
    qam->type = ACK_MSG;
    qam->client_ack_delay = client_ack_delay;
    qam->lb_recv_timestamp = ngx_current_msec * 1000000;
    qam->lb_send_timestamp = ngx_current_msec * 1000000;
    qam->pn_min = 0;
    qam->pn_max = 0;
    if (ack_range_count > 0) {
        u_char *last = ngx_cpymem(&qam->ack_min, tmp_buf, ack_range_count * 2 * sizeof(uint64_t));
        assert(last == ack_buf->last);
    }

#ifdef ACK_DEBUGBUF
    snprintf((char *)pt, debugbuf + sizeof(debugbuf) - pt, "ack mp total len %lu, qsid %ld, pnsid %lu, %lu",
            total_len, qs->id, qs->dsr_cid->seqnum, ngx_current_msec);
    ngx_log_error(NGX_LOG_ERR, c->log, 0, "%s, %p", debugbuf, u->peer.connection);
#endif

    qsho_upstream_write_handler(r, u);
...
{
    ngx_quic_stream_t *qs = c->quic;
    ngx_connection_t *pc = qs->parent;
    ngx_quic_connection_t *qc = ngx_quic_get_connection(pc);
    ngx_queue_t            *q;
    ...
    {   
        ...
        p = ngx_hex_dump(tmp_buf, item->id, item->len);
        snprintf((char *)p, tmp_buf + sizeof(tmp_buf) - p, " dsr cid type %d, ref %lu, qsid %ld, pnsid %lu", (int)item->type, item->refcnt, qs->id, qs->dsr_cid->seqnum);
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "%s", tmp_buf);
#endif
...
ngx_chain_t *qsho_global_free_chain = NULL;

ngx_chain_t *
alloc_qsho_buf_chain_buf()
{
    if (qsho_global_free_chain == NULL) {
        ngx_chain_t *chains;
        ngx_buf_t   *bufs;
        ngx_chain_t **tail = &qsho_global_free_chain;
        ngx_uint_t i;
        chains = ngx_calloc(128 * sizeof(ngx_chain_t), ngx_cycle->log);
        bufs = ngx_calloc(128 * sizeof(ngx_buf_t), ngx_cycle->log);
        for (i = 0; i < 128; i ++) {
            *tail = &chains[i];
            (*tail)->next = NULL;
            (*tail)->buf = &bufs[i];

            bufs[i].start = ngx_calloc(4096, ngx_cycle->log);
            bufs[i].temporary = 1; 
            bufs[i].tag = "qsho buf";
            bufs[i].pos = bufs[i].start;
            bufs[i].last = bufs[i].start;
            bufs[i].end = bufs[i].start + 4096;

            tail = &((*tail)->next);
        }
    }

    ngx_chain_t *ret = qsho_global_free_chain;
    qsho_global_free_chain = qsho_global_free_chain->next;
    ret->next = NULL;
...
void
free_qsho_buf_chain(ngx_chain_t *it)
{
    assert(it->buf->start);
    it->buf->pos = it->buf->start;
    it->buf->last = it->buf->start;
    it->buf->end = it->buf->start + 4096;
    it->buf->temporary = 1;

    it->next = qsho_global_free_chain;
    qsho_global_free_chain = it;
}
