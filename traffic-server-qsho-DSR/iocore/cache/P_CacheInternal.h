...
#define VC_LOCK_RETRY_EVENT()                                                                                         \
  do {                                                                                                                \
        Note("%s:%d VC_LOCK_RETRY_EVENT retry, %ld, %d", __FILE__, __LINE__, Thread::get_hrtime(), cache_config_mutex_retry_delay);    \
...
#define VC_SCHED_LOCK_RETRY()                                                                                  \
  do {                                                                                                         \
    Note("%s:%d try lock failed, retry, %ld, %d", __FILE__, __LINE__, Thread::get_hrtime(), cache_config_mutex_retry_delay);    \
...
#define CONT_SCHED_LOCK_RETRY_RET(_c)                                                                  \
  do {                                                                                                 \
    Note("%s:%d CONT_SCHED_LOCK_RETRY_RET, retry, %ld, %d", __FILE__, __LINE__, Thread::get_hrtime(), cache_config_mutex_retry_delay);    \
...
#define CONT_SCHED_LOCK_RETRY(_c) \
        _c->mutex->thread_holding->schedule_in_local(_c, HRTIME_MSECONDS(cache_config_mutex_retry_delay)); \
        Note("%s:%d CONT_SCHED_LOCK_RETRY, retry, %ld, %d", __FILE__, __LINE__, Thread::get_hrtime(), cache_config_mutex_retry_delay);

#define VC_SCHED_WRITER_RETRY()                                           \
  do {                                                                    \
    Note("%s:%d VC_SCHED_WRITER_RETRY, retry, %ld, %d", __FILE__, __LINE__, Thread::get_hrtime(), cache_config_mutex_retry_delay);    \
    ink_assert(!trigger);                                                 \
    writer_lock_retry++;                                                  \
...
CacheVC::do_read_call(CacheKey *akey)
{
  ...
  //Note("do_read_call aio_nbytes %lu", io.aiocb.aio_nbytes);
