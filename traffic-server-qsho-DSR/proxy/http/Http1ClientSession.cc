...
Http1ClientSession::state_keep_alive(int event, void *data)
{
  ...
  case VC_EVENT_READ_READY:
    ...
    // request
    //Note("Http1ClientSession::state_keep_alive VC_EVENT_READ_READY");
    new_transaction();
    break;
...
Http1ClientSession::release(ProxyTransaction *trans)
{
  ...
  if (more_to_read) {
    ...
    //Note("[%" PRId64 "] data already in buffer, starting new transaction %p %p, %ld", con_id, this, read_buffer, read_buffer->write_avail());
    new_transaction();
  } else {
    HttpSsnDebug("[%" PRId64 "] initiating io for next header", con_id);
    //Note("[%" PRId64 "] initiating io for next header %p %p, %ld", con_id, this, read_buffer, read_buffer->write_avail());
