...
static void ngx_quic_compute_nonce_with_pnsid(u_char *nonce, size_t len, uint64_t pn, uint64_t pnsid);
...
ngx_quic_create_packet(ngx_quic_header_t *pkt, ngx_str_t *res)
{
    ...
    if (pkt->level == ssl_encryption_application) {
        //ngx_quic_compute_nonce_with_pnsid(nonce, sizeof(nonce), pkt->number, pkt->pnsid);
        ngx_quic_compute_nonce_with_pnsid(nonce, sizeof(nonce), pkt->number, 0);
#if 0
        unsigned char tmp_buf[32];
        memset(tmp_buf, 0, sizeof(tmp_buf));
        ngx_hex_dump(tmp_buf, nonce, 12);
        ngx_log_error(NGX_LOG_ERR, pkt->log, 0, "nonce %s %uL %uL", tmp_buf, pkt->number, pkt->pnsid);
#endif
    } else {
        ngx_quic_compute_nonce_with_pnsid(nonce, sizeof(nonce), pkt->number, 0);
    }
...
static void
ngx_quic_compute_nonce_with_pnsid(u_char *nonce, size_t len, uint64_t pn, uint64_t pnsid)
{
