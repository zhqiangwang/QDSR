...
public:
  ...
  VConnection *do_qsho_transform_open();
  void do_qsho_transform_close();
...
protected:
  ...
  int tunnel_handler_qsho_ua(int event, HttpTunnelConsumer *c);
  ...
  int write_qsho_http3_header_into_buffer(HTTPHdr *h, MIOBuffer *b);
...
HttpSM::write_response_header_into_buffer(HTTPHdr *h, MIOBuffer *b)
{
  if (t_state.client_info.http_version == HTTPVersion(0, 9)) {
    return 0;
  } else if (t_state.qsho_transform && t_state.source == HttpTransact::SOURCE_CACHE) {
    return write_qsho_http3_header_into_buffer(h, b);
  } else {
