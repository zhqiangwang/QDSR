...
public:
  ...
  struct State {
    ...
    unsigned char *quic_connection_id = nullptr;
    uint32_t quic_connection_id_len = 0;
    uint64_t packet_number;
    uint64_t packet_number_max;
    int64_t lb_send_timestamp;
    uint64_t quic_stream_id;
    uint64_t nginx_worker_id;
    uint32_t src_ip;
    uint16_t src_port;
    uint32_t dst_ip;
    uint16_t dst_port;
    ...
    {
      //qsho destroy
      if (quic_connection_id) {
        ats_free(quic_connection_id);
        quic_connection_id = nullptr;
