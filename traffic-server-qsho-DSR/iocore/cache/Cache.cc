...
CacheVC::reenable(VIO *avio)
{
  ...
  if (!trigger) {
    ...
    //Note("cache_reenable reenable, schedule imm %ld", Thread::get_hrtime());
    trigger = avio->mutex->thread_holding->schedule_imm_local(this);
  } else {
#ifdef DEBUG
    //Note("had trigger111111111111111, do nothing %ld", Thread::get_hrtime());
#endif
  }
}
...
CacheVC::handleReadDone(int event, Event *e)
{
  ...
  //Note("Cache VC handleReadDone %ld, %p %s", Thread::get_hrtime(), (void *)&CacheVC::handleReadDone, __func__);

  Doc *doc = nullptr;
  if (event == AIO_EVENT_DONE) {
    set_io_not_in_progress();
  } else if (is_io_in_progress()) {
    //Note("Cache VC handleReadDone, IO in progress");
    return EVENT_CONT;
  }
  {
...
#if 0
      Note("cache read done, prepare save in ram");
#endif
      int okay = 1;
...
CacheVC::handleRead(int /* event ATS_UNUSED */, Event * /* e ATS_UNUSED */)
{
  ...
  //Note("cachevc handle read %ld, ram hit state %d", Thread::get_hrtime(), ram_hit_state);
...
{
  //Note("Cache VC removeEvent %ld, %p %s", Thread::get_hrtime(), (void *)&CacheVC::removeEvent, __func__);
  cancel_trigger();
  set_io_not_in_progress();
  {
    MUTEX_TRY_LOCK(lock, vol->mutex, mutex->thread_holding);
